...
function [air_matrix, onlyTissue]=visualize_hotspots_histograms_temperature(tissueMatrix, input_T_matrix, tissue_filepath)
...
T_matrix=input_T_matrix+37;

Tmax=max(T_matrix(:));

onlyTissue=ones(size(input_T_matrix));
air_matrix=zeros(size(input_T_matrix));
...
onlyTissueT=T_matrix.*onlyTissue;
T_matrix=onlyTissueT;
% ---- Default parameters ----
slice=round(size(tissueMatrix,3)/2); % Set center slice as default slice 
HSscaling=1;                        % Set default Color scale as 5
HSslideMax=Tmax;              % Set default Hot Spot slider maximum
...
f1=subplot(1,2,1); imageOverlay(tissueMatrix,T_matrix, slice, display_limit, HSscaling,cut);
...
setappdata(f,'sm',T_matrix);
setappdata(f,'sarmax',Tmax);
...
expText = uicontrol('Parent',p,...
                    ...
                    'String',sprintf(['This program displays all points in the temperature matrix with values above the display limit.',...
                            '\n Change the display limit by using the slider.\n ',...
                            'Adjust the color scale on the temperature values on display by using the Color Scaling slider. \n '...
                            , '']),...
                    'BackgroundColor',get(f,'Color'));   
...
result=getResultString(getappdata(f,'tm'), onlyTissueT);
...
function hotspotButt(hObj,event,hsfig) %#ok<INUSL>
    ...
    [output]=findHotspot(onlyTissueT, str2double(get(getappdata(hsfig,'nhsb1'),'string')), str2double(get(getappdata(hsfig,'hslb1'),'string')), str2double(get(getappdata(hsfig,'nnb1'),'string')));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% getResultString FUNCTION %%%%%%%%%%%%%%%%%%%%%%%
    ...
    function result=getResultString(tissue_matrix, T_matrix)

        target_matrix=((tissue_matrix==tumorValue)+ (tissue_matrix==cystTumorValue)).*T_matrix;
        nbr_tum_elements=length(find(target_matrix));
        tumor_vector=sort(reshape(target_matrix,[size(target_matrix,1)*size(target_matrix,2)*size(target_matrix,3),1]),'descend');
        tumor_vector=tumor_vector(1:nbr_tum_elements);
        
        T_max=tumor_vector(1);
        
        
        T90=tumor_vector(round(0.9*length(tumor_vector)));
        T50=tumor_vector(round(0.5*length(tumor_vector)));
        T10=tumor_vector(round(0.1*length(tumor_vector)));
        % Concatenate string
        result=sprintf(['Max temperature in tumor = ', num2str(T_max),...
            '\n T90 = ', num2str(T90), ...
            '\n T50 = ', num2str(T50),...
            '\n T10 = ', num2str(T10) ]);
    end
    ...
    function imageOverlay(tissueMatrix,Tmatrix, slice, HSvalue, HSscaling,cutPlane)
            ...
            if cutPlane=='X'
                ...
                B=reshape(Tmatrix(slice,:,:),[size(Tmatrix,2) size(Tmatrix,3)]);
            ...
            elseif cutPlane=='Y'
                ...
                B=reshape(Tmatrix(:,slice,:),[size(Tmatrix,1) size(Tmatrix,3)]);
                xlimit=[1, size(tissueMatrix,3)] ;
    ...
    function plotHTQHistograms(TissueMatrix,PLDMatrix,tissuePlot)
        ...
        healthyMatrix=T_matrix.*healthyTissue;
        ...
        tumorTissue=(B== tumorValue).*T_matrix;
        cystTumorTissue=zeros(size(B));
        if ~isempty(cystTumorIndex)
            cystTumorTissue=(B== cystTumorValue).*T_matrix;
        end
        ...
        for i=1:length(tissuePlot)
            ...
            plotMatrix=plotTissue.*T_matrix;
        ...
        xlabel('Percentage of maximum T in tumor tissue');
        ylabel('Cumulative volume');
        title(['Maximum T in tumor = ',int2str(sortTumorVector(1))]);
        axes(getappdata(f,'f4'));
        legend([tissueLegend(tumorIndex), tissueLegend{tissueLegendVector}])
        xlabel('Percentage of maximum T in healthy tissue');
        ylabel('Cumulative volume');
        title(['Maximum T in healthy tissue = ',int2str(sortHealthyVector(1))]);
        axis([0,100,0,1]);
