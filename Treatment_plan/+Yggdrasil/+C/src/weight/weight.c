...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    /* Check dimensions */
    if (octB.data_dim != 1)
        mexErrMsgTxt("The weight needs to be a scalar field.");
    ...
    octree octWeighted = weight(octA, octB);
    
#ifdef DEBUG  
    oct_print(&octWeighted);
#endif
    
    oct_to_output(&OUT_DATA, &OUT_ADR, &OUT_META, &octWeighted);
}

// Actual work
octree weight(octree octA, octree octB)
{
    ...
    for(INDEX(octA) = INDEX(octB) = INDEX(octC) = 0;;) {
        ...
        cv_weight(&octC.data, &octA.data, &octB.data);
        ...
        if (ADR(octA) == ADR(octB)) {
            ...
            // End condition, is octA, octB at end adress?
